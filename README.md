...
## Notes on the work performed for the Topics in Intelligent Systems curricular unit
...
Note that, after this initial portion of the README that contains the additional work performed during this course, the rest of the README is from the forked repository.

It has references to files that we renamed. The original files from the paper's code were renamed from `multiagent.py` to `multiagent_paper.py` (training script) and from `test_multiagent.py` to `test_multiagent_paper.py` (testing script).

### Installing libraries
The work was reproduced and modified using Python version 3.8.10. The pip version was 21.1.1 (adequate for the Python version). To install the libraries in the exact version used for the work results reproduction, you can run `pip install -e .` on the root of the repository.

If any issues arise, consult the `requirements.txt` file for the complete list of the versions of installed libraries (contains the versions for secondary libraries installed through the requirements of the main libraries).

To create the plots used for the presentation, the Logger [gym_pybullet_drones/utils/Logger.py](gym_pybullet_drones/utils/Logger.py) was adapted. Additionally, new plots were created using the files generated by training.
To facilitate generating these plots, we decided to use modern libraries, so the requirements are different. These are presented in the `requirements_for_plots.txt`.


### General notes on working with multiagent RL in this repository
To work or test all multiagent examples, you should first move to the [experiments/learning/](experiments/learning) folder: `cd experiments/learning`.
...
Since the other examples from the paper are not multiagent, they were not shown here. The way to run these examples is presented in the paper, but not placed here since they are not multiagent and hence not the focus of this work.

### New environments
- To run training scripts, you may sometimes need to specify the environment, number of drones
and action space.
- For the testing scripts, you just need to specify the correct folder inside
[experiments/learning/results](experiments/learning/results).
This folder's name has the environment, number of drones, algorithm, observation space and action space.
...
```bash
python multiagent_one_policy.py --env meet_at_height --act one_d_rpm --num_drones 5
```
But we can also test training different policies to compare performances. This requires running a different script to train.
```bash
python multiagent_meet_at_height_many_policies.py
```

To test the environment on just one trained policy:
```bash
...
To test the environment on multiple trained policies:
```bash
python test_multiagent_many_policies.py --exp ./results/save-meet_at_height-5-cc-kin-one_d_rpm-{month}.{day}.{year}_{hour}.{minute}.{second}/
```

Note that the name of the folder has the same format as the previous so you need to note which runs were performed on just one or multiple policies.
...
This example can only be run with 2 drones. It also requires training with different policies, so a different training script should be used.

```bash
python multiagent_chase_many_policies.py
```

This example requires different policies, so to run it, we use a different script:
...
These are 4D action spaces, which are much harder for the agents to learn.

In fact, this was an unsuccessful environment, where the drones
failed to learn the desired policy.

For this environment, you can specify different number of drones,
although, since it fails, there is no added value. Further details are on the [presentation](presentation.pdf).
```bash
python multiagent_one_policy.py --env figure --num_drones 2 --act rpm
```
```bash
python multiagent_one_policy.py --env figure --num_drones 2 --act dyn
```

To test the learned policy:
```bash
python test_multiagent_one_policy.py --exp ./results/save-figure-2-cc-kin-{act}-{month}.{day}.{year}_{hour}.{minute}.{second}/
```

The parameter `act` represents the action space should be either `rpm` or `dyn` (depending on how the agents were trained).


## Forked Project's README
