...
If any issues arise, consult the `requirements.txt` file which contains the complete list of the versions of installed libraries (contains the versions for secondary libraries installed through the requirements of the main libraries).
...
```
- Testing: This example requires different policies, so to run it, we use a different script:
```bash
...
In fact, these are unsuccessful environments, where the drones
fail to learn the desired policy.
For this environment, you can specify different number of drones,
although, since it fails, there is no added value. Further details are on the presentation.
```bash
...
```bash
python test_multiagent_one_policy.py --exp ./results/save-figure-2-cc-kin-one_d_rpm-{month}.{day}.{year}_{hour}.{minute}.{second}/
```
