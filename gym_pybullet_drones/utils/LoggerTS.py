...
class Logger(object):
    ...
    def plot(self, pwm=False):
        """Plots the stored data including states and rewards."""
        # We will now have 4 subplots: z, vz, RPM, and reward.
        rows = 1
        cols = 1
        colors = ['red', 'blue']  # Adjust if you have more drones
        ...
        # row = 1
        # for j in range(self.NUM_DRONES):
        #     axs[row].plot(t, self.states[j, 5, :], label="drone_"+str(j))
        # axs[row].set_xlabel('time (s)')
        # axs[row].set_ylabel('vz (m/s)')

        # #### RPM plot
        # row = 2
        # for j in range(self.NUM_DRONES):
        #     axs[row].plot(t, self.states[j, 12, :], label="drone_"+str(j))
        # axs[row].set_xlabel('time (s)')
        # axs[row].set_ylabel('RPM 0')

        #### Rewards plot
        row = 0
        for j in range(self.NUM_DRONES):
            axs[row].plot(t, self.rewards[j, :], label="drone_"+str(j))
        axs[row].set_xlabel('time (s)')
        ...
        fig.legend([f"drone_{i}" for i in range(self.NUM_DRONES)], loc="lower center", ncol=self.NUM_DRONES)
        plt.tight_layout()
